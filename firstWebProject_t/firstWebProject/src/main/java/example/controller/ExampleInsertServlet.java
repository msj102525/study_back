...
import example.model.vo.ExampleVO;

/**
 * Servlet implementation class ExampleInsertServlet
 */
@WebServlet("/exinsert")
public class ExampleInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public ExampleInsertServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		vo.setUserName(mrequest.getParameter("username"));
		vo.setGender(mrequest.getParameter("gender"));
		vo.setAge(Integer.parseInt(mrequest.getParameter("age")));

		// 6. 업로드된 원본 파일이름 추출
		String originalFileName = mrequest.getFilesystemName("myphoto");		
		...
		if (originalFileName != null) {
			...
			renameFileName += "_" + vo.getUserName()
					+ "." + originalFileName.substring(originalFileName.lastIndexOf(".") + 1);

			vo.setMyPhoto(renameFileName);
		...
		System.out.println("exinsert : " + vo);
		...
		//임시로 페이지 이동 처리함
		response.sendRedirect("/first/views/example/sampleList.html");
	}
