...
public class EnrollServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 회원가입 처리용 컨트롤러
		// *** 사진 파일 첨부시 multipart 전송 처리방식으로 코드 변경함 ***
		// photo_userid.확장자로 이름 변경하여 폴더에 저장 처리함
		...
		System.out.println("photo : " + request.getParameter("photofile"));
		
		//checkbox 전송형태 : 
		//...&hobby=game&hobby=sport&hobby=cook&.....
		//같은 이름의 값이 여러개 전송온 경우 getParameter("이름") 사용하면 에러남
		// getParameterValues("이름") : String[] 사용함
		String[] hobbies = request.getParameterValues("hobby");
		//System.out.println("hobbies.length : " + hobbies.length);
		//문자열 배열을 하나의 String  으로 변환한다면 (구분자를 ,로 정한 경우)
